...
BitWriter* BitWriter__new(FILE* f) {
    ...
    bw->f = f;
    bw->buffer = malloc(sizeof(char) * BUFFER_SIZE);
    bw->current_byte = 0;
    bw->byte_offset = 0;
    bw->buffer_offset = 0;
    return bw;
...
void BitWriter__write_bit(BitWriter* this, char* bit) {
    ...
    if (this->byte_offset == 8) {
        ...
        if (this->buffer_offset == BUFFER_SIZE) {
            fwrite(this->buffer, sizeof(char), BUFFER_SIZE, this->f);
...
void BitWriter__write_buffer(BitWriter* this) {
    ...
    fwrite(this->buffer, sizeof(char), this->buffer_offset, f);
}
